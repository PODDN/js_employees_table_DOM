...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    const textToMoneyNumber = (text)=>Number(String(text).replace(/[^\d.-]/g, "")) || 0;
    const table = $("table");
    ...
    function sortBy(index, dir) {
        ...
        const compare = (a, b)=>{
            ...
            if (isNumA && isNumB) return (Number(sa) - Number(sb)) * dir;
            return vaRaw.localeCompare(vbRaw, undefined, {
                sensitivity: "base"
            }) * dir;
        };
        rows.sort(compare).forEach((tr)=>tbody.appendChild(tr));
    }
    const headerIndex = (th)=>[
    ...
    function onHeaderClick(e) {
        ...
        if (currentSort.index === index) sortBy(index, -currentSort.dir);
        else sortBy(index, 1);
    }
    ...
    table.addEventListener("click", (e)=>{
        if (e.target.closest("thead th") || e.target.closest("tfoot th")) onHeaderClick(e);
    });
    table.addEventListener("dblclick", (e)=>{
        if (e.target.closest("thead th") || e.target.closest("tfoot th")) onHeaderDblClick(e);
    });
    ...
    const pushNotification = (posTop, posRight, title, description, type)=>{
        ...
        n.style.backgroundColor = type === "error" ? "rgba(255,0,0,0.7)" : "rgba(0,128,0,0.7)";
        n.style.color = "white";
    ...
    function endEdit(save) {
        ...
        if (!save || newVal === "") td.textContent = oldText;
        else {
            if (td.cellIndex === 3) {
